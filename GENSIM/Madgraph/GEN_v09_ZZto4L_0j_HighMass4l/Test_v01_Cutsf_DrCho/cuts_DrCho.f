...
c-  sum over the momenta
      ...
      ! <CP-WS; calc 4-lepton inv mass and apply cut
      do j=0,3
         p4l(j)=0  
      enddo     
      ...
      do i=nincoming+1,nexternal
         if (is_a_l(i)) then
            do j=0,3
               p4l(j)=p4l(j)+p(j,i); ! sum of lepton momenta
            enddo
         endif
      enddo

      m4l=dsqrt(SumDot(p4l,p4l,1d0))
      mm4l=1000.0
      mm4lmax=2000.0

      if (mm4l.gt.0d0.or.mm4lmax.ge.0d0)then
         if(m4l.lt.mm4l.or.mm4lmax.ge.0d0.and.m4l.gt.mm4lmax) then
            if(debug) write (*,*) '4 lepton invariant mass -> fails'
            passcuts=.false.
            return
         endif
      endif
